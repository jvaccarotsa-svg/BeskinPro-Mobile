...
// Robust Spanish word boundary matching that handles accents AND common variations/slang
function matchesWord(text, words) {
    if (!text) return false;

    // Normalize text: lowercase, remove accents, and trim
    // Using NFD + regex is standard for removing diacritics
    const t = text.toLowerCase().trim()
        .normalize("NFD").replace(/[\u0300-\u036f]/g, "")
        .replace(/[.,?!¿¡]/g, " "); // Replace punctuation with space to avoid boundary issues

    const baseWords = Array.isArray(words) ? words : [words];

    // Normalize target words
    const normalizedWords = baseWords.map(w =>
        w.toLowerCase().trim().normalize("NFD").replace(/[\u0300-\u036f]/g, "")
    );

    return normalizedWords.some(word => {
        // Use more robust word boundary matching
        // Also allow matching if the word is exactly the text after trim
...
// Centralized affirmative/negative lists for better consistency
const YES_WORDS = ['si', 'sí', 'claro', 'por supuesto', 'afirmativo', 'correcto', 'exacto', 'totalmente', 'bastante', 'mucho', 'siempre', 'sip', 'si si'];
const NO_WORDS = ['no', 'nunca', 'nada', 'ningun', 'ninguno', 'ninguna', 'jamas', 'para nada', 'nop', 'no no'];
const SOME_WORDS = ['poco', 'algo', 'a veces', 'un poco', 'alguna', 'algunas', 'pocas', 'leves', 'moderada', 'relativamente'];
...
export const BAUMANN_QUESTIONS = [
    {
        ...
        parseResponse: (text) => {
            if (matchesWord(text, ['mucho', 'bastante', ...YES_WORDS])) return 8;
            if (matchesWord(text, ['poco', 'algo', 'a veces', 'un poco'])) return 4;
            if (matchesWord(text, NO_WORDS)) return 1;
            return null;
        }
    },
    {
        ...
        parseResponse: (text) => {
            if (matchesWord(text, ['mucho', 'muchos', 'siempre', 'grasa', 'graso', ...YES_WORDS])) return 8;
            if (matchesWord(text, SOME_WORDS)) return 4;
            if (matchesWord(text, NO_WORDS)) return 1;
            return null;
        }
    },
    {
        ...
        parseResponse: (text) => {
            if (matchesWord(text, ['ardor', 'picor', 'irritacion', 'siempre', 'mucho', 'frecuentemente', ...YES_WORDS])) return 8;
            if (matchesWord(text, SOME_WORDS)) return 4;
            if (matchesWord(text, NO_WORDS)) return 1;
            return null;
        }
    },
    {
        ...
        parseResponse: (text) => {
            if (matchesWord(text, ['tengo', 'muchas', 'muchos', 'bastante', 'manchas', 'marcas', ...YES_WORDS])) return 8;
            if (matchesWord(text, SOME_WORDS)) return 4;
            if (matchesWord(text, NO_WORDS)) return 1;
            return null;
        }
    },
    {
        ...
        parseResponse: (text) => {
            if (matchesWord(text, ['muchas', 'muchos', 'marcadas', 'tersura', 'bastantes', ...YES_WORDS])) return 8;
            if (matchesWord(text, SOME_WORDS)) return 4;
            if (matchesWord(text, NO_WORDS)) return 1;
            return null;
        }
    },
    {
        ...
        parseResponse: (text) => {
            if (matchesWord(text, ['mucho', 'frecuentemente', 'alta', 'bastante', ...YES_WORDS])) return true;
            if (matchesWord(text, ['nunca', 'poco', 'baja', 'moderada', ...NO_WORDS])) return false;
            return null;
        }
    },
    {
        ...
        parseResponse: (text) => {
            if (matchesWord(text, ['reaccion', 'alergia', 'intolerancia', 'varias', ...YES_WORDS])) return 8;
            if (matchesWord(text, ['una vez', 'alguna', 'alguna vez', ...SOME_WORDS])) return 4;
            if (matchesWord(text, NO_WORDS)) return 1;
            return null;
        }
    },
    {
        ...
        parseResponse: (text) => {
            if (matchesWord(text, ['mucho', 'bastante', 'descama', 'pela', ...YES_WORDS])) return 8;
            if (matchesWord(text, SOME_WORDS)) return 4;
            if (matchesWord(text, NO_WORDS)) return 1;
            return null;
        }
...
export class VoiceChatbot {
    constructor(onStateChange, onAnswer, language = 'es-ES') {
        ...
        this.audioCtx = typeof window !== 'undefined' ? new (window.AudioContext || window.webkitAudioContext)() : null;
    ...
    playBeep() {
        ...
        try {
            ...
            console.warn('[VoiceChatbot] Could not play beep:', e);
        }
    }
